...
class BertModel(object):
  ...
  def __init__(self,
    ...
    def enable_dropout():
      return (orig_hidden_dropout_prob, orig_attention_probs_dropout_prob)
        
    config.hidden_dropout_prob, config.attention_probs_dropout_prob = tf.cond(is_training, true_fn=enable_dropout, false_fn=disable_dropout)
...
def transformer_model(input_tensor,
  ...
  for layer_idx in range(num_hidden_layers):
    with tf.compat.v1.variable_scope("layer_%d" % layer_idx):
      ...
      # Down-models back to `hidden_size` then add the residual.
      with tf.compat.v1.variable_scope("output"):
