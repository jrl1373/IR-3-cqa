...
tf.io.gfile.makedirs(FLAGS.output_dir + '/summaries/train/')
tf.io.gfile.makedirs(FLAGS.output_dir + '/summaries/val/')
tf.io.gfile.makedirs(FLAGS.output_dir + '/summaries/rl/')
...
# tf Graph input
unique_ids = tf.compat.v1.placeholder(tf.int32, shape=[None], name='unique_ids')
input_ids = tf.compat.v1.placeholder(tf.int32, shape=[None, FLAGS.max_seq_length], name='input_ids')
input_mask = tf.compat.v1.placeholder(tf.int32, shape=[None, FLAGS.max_seq_length], name='input_mask')
segment_ids = tf.compat.v1.placeholder(tf.int32, shape=[None, FLAGS.max_seq_length], name='segment_ids')
start_positions = tf.compat.v1.placeholder(tf.int32, shape=[None], name='start_positions')
end_positions = tf.compat.v1.placeholder(tf.int32, shape=[None], name='end_positions')
history_answer_marker = tf.compat.v1.placeholder(tf.int32, shape=[None, FLAGS.max_seq_length], name='history_answer_marker')
training = tf.compat.v1.placeholder(tf.bool, name='training')
get_segment_rep = tf.compat.v1.placeholder(tf.bool, name='get_segment_rep')
...
with tf.compat.v1.Session() as sess:
    ...
    if FLAGS.do_train:
        train_summary_writer = tf.compat.v1.summary.FileWriter(FLAGS.output_dir + 'summaries/train', sess.graph)
        val_summary_writer = tf.compat.v1.summary.FileWriter(FLAGS.output_dir + 'summaries/val')
